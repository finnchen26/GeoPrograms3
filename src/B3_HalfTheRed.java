...
public class B3_HalfTheRed extends World {
    ...
    public void go() {
        ...
        for (int row = 152; row < 303; row = row + 1) {
            for (int col = 0; col < 506; col++) {
...
//                plane.teleport(col, 488 - row);
                if (red < 245 && green < 158 && blue < 54) {
                    System.out.println("red not zero");
                    plane.setPixelColor(130, 210, 240);


                }
                else {
                    plane.setPixelColor(col / 4, row/2, 0);
